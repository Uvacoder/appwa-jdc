...
export function createCssGradientString(colors: string[]) {
  if (colors.length > 0) {
    if (colors.length === 1) return { backgroundColor: colors[0] }
    else {
      let colorSegment: string = ''
      const percentage = 100 / colors.length

      colors.forEach(
        (color, i) => (colorSegment += `, ${color} ${percentage}%`)
      )

      return { background: `linear-gradient(-45deg${colorSegment})` }
    }
  } else {
    return {}
  }
